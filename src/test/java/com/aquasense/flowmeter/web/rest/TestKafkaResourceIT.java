...
import com.aquasense.flowmeter.config.KafkaProperties;
...
class TestKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/test-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/test-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
